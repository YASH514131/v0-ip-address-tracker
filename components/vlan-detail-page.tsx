...
interface VlanDetailPageProps {
  vlan: VLAN
  colorIndex: number
  onBack: () => void
}

export function VlanDetailPage({ vlan, colorIndex, onBack }: VlanDetailPageProps) {
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <Badge variant="outline" className={`${colors.text} ${colors.border} text-lg px-3 py-1`}>
              VLAN {vlan.vlanId}
            </Badge>
        ...
        {vlan.description && <p className="text-muted-foreground">{vlan.description}</p>}

        {/* Stats row */}
        <div className="flex gap-4">
          <Card className="flex-1">
            <CardContent className="pt-4">
              <div className="text-2xl font-bold">{vlanDevices.length}</div>
              <div className="text-sm text-muted-foreground">Devices</div>
            </CardContent>
          </Card>
          <Card className="flex-1">
            <CardContent className="pt-4">
              <div className="text-2xl font-bold">{vlanIps.length}</div>
          ...
          <Card className="flex-1">
            <CardContent className="pt-4">
              <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                {vlanIps.filter((ip) => ip.status === "available").length}
              </div>
          ...
          <Card className="flex-1">
            <CardContent className="pt-4">
              <div className="text-2xl font-bold text-sky-600 dark:text-sky-400">
                {vlanIps.filter((ip) => ip.status === "assigned").length}
              </div>
